...
namespace nitou.LevelActors.Controller.Smb {
    ...
    public class AnimatorMoveBehaviour : StateMachineBehaviour {
        ...
        [Tooltip("Duration to raise the priority of AnimationMove")]
        private float _start = 0, _end = 1;

        [HideInInspector] 
        [SerializeField] private bool _copyed = false;
        ...
        /// </summary>
        [SerializeField]
        [Tooltip("Determines whether to use ground detection when moving the character with AnimationMove. For example, set to False when moving up and down like on a ladder.")]
